...
const activeMarkets = ref<ActiveSymbol[]>([])
const poolKeyword = ref('')
let timer: number | undefined

const filteredMarkets = computed(() => {
  const keyword = poolKeyword.value.trim().toUpperCase()
  if (!keyword) return activeMarkets.value
  return activeMarkets.value.filter((market) =>
...
/** 打开交易池时获取最新明细，避免概览轮询反复传输整张列表。 */
async function openActivePool() {
  poolVisible.value = true
  poolLoading.value = true
  try {
    activeMarkets.value = await api.activeMarkets()
  } catch (error) {
    ElMessage.error(errorMessage(error))
  } finally {
...
<template>
  ...
  <div class="metric-grid" v-loading="loading && !stats">
    ...
    <div class="metric"><p class="metric-label">活跃交易池</p><button class="metric-value metric-value-button" type="button" aria-label="查看活跃交易池明细" @click="openActivePool">{{ stats?.active_symbols ?? '—' }}</button><div class="metric-note">通过 24h 成交额过滤 · 点击查看明细</div></div>
  ...
  <el-drawer v-model="poolVisible" title="活跃交易池" size="min(860px, 100%)" destroy-on-close>
    <div class="pool-toolbar">
      <span>共 {{ activeMarkets.length }} 个交易对</span>
      <el-input v-model="poolKeyword" :prefix-icon="Search" clearable placeholder="搜索交易对" aria-label="搜索交易对" />
    </div>
    <el-table v-loading="poolLoading" :data="filteredMarkets" height="calc(100vh - 170px)" stripe empty-text="暂无活跃交易对">

...
class FakeBinanceClient:
    ...
    async def open_interest(self, symbol: str, timeframe: str, start_ms: int):
        """返回覆盖当前 K 线起点和最新时刻的两个 OI 点。"""
        self.calls += 1
        return [
            OIPoint(timestamp=self.start, open_interest=Decimal("1000")),
            OIPoint(timestamp=datetime.now(timezone.utc) - timedelta(seconds=1), open_interest=Decimal("1001")),
        ]
...
def make_kline(start: datetime, volume: str, closed: bool) -> KlineData:
    """构造当前或完整 15 分钟 K 线。"""
    return KlineData(
        symbol="BTCUSDT", timeframe="15m", open_time=start,
        close_time=start + timedelta(minutes=15), open=100, high=102, low=99, close=101,
...
async def test_signal_requires_both_volume_and_oi_and_is_published() -> None:
    ...
    cache = KlineCache()
    history = [make_kline(start - timedelta(minutes=15 * index), "100", True) for index in range(60, 0, -1)]
    await cache.initialize("BTCUSDT", "15m", history + [make_kline(start, "200", False)])
    client = FakeBinanceClient(start)
    published = []
    ...
    run = await scanner.scan({"BTCUSDT"}, tickers, ConfigValues(timeframes=["15m"]))
